...
while True:
    try:
        ...
        if len(contourg)>0:
            c1 = max(contourg,key=cv2.contourArea)
            ((x1, y1), radius1) = cv2.minEnclosingCircle(c1)
        elif len(contourg2)>0:
            c2 = max(contourg2,key=cv2.contourArea)
        ...
        if len(contours)<1 or cv2.contourArea(max(contours,key=cv2.contourArea))<20:
            ...
            if int(x1)<split and goal == 1:
                left.setSpeed(40)
                right.setSpeed(90)
            elif int(x1)>split and goal == 1:
                right.setSpeed(40)
                left.setSpeed(90)
            elif int(x2)<split and goal == 2:
                left.setSpeed(40)
                right.setSpeed(90)
            elif int(x2)>split and goal == 2:
                right.setSpeed(40)
